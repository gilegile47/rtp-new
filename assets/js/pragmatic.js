...
var random = ["✔️✖️✖️", "✖️✔️✖️", "✖️✖️✔️", "✔️✖️✔️", "✖️✔️✔️", "✖️✖️✖️", "✔️✔️✔️", "✔️✔️✖"];
...
for (let i = 0; i <= text.length; i++) {
	...
	$(barr[i])
		.attr("aria-valuenow", xx).css("width", xx + "%");
	if (xx < 30) {
	...
	if (xx >= 30) {
		...
		for (let j = 0; j < 3; j++) {
			if (random3[p3[j]] == "Auto") {
				pola1[j][i].innerText = random[p2[j]] + " " + random3[p3[j]];
			} else {
		...
		pola1[1][i].innerHTML = "NOT RECOMMEN!";
...
function plusSlides(value){
	var mowidth = $('#myoverflow').width();
	var positionnav = $('#positionnav').val();
	//alert(mowidth);
	if(value>0){
		if(positionnav < 15){
			lastpost = positionnav * 32;
			$('#myoverflow').scrollLeft(lastpost + 32);
			$('#positionnav').val(parseInt(positionnav) + 1);
		}
	}else{
		if(positionnav > 0){
			lastpost = positionnav * 32;
